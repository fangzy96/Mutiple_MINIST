...
if __name__ == '__main__':
    ...
    tuned_parameters = dict(C=randint(1, 10000), kernel=['linear', 'poly', 'rbf', 'sigmoid'])
    svm_clf = RandomizedSearchCV(SVC(cache_size=1000), tuned_parameters, scoring="accuracy", n_jobs=-1)
    ...
    scores = cross_val_score(svm_clf.best_estimator_, X_test, y_test, scoring="accuracy", cv=5)
    print(scores)
    ...
    clf = SVC()
    clf.set_params(**svm_clf.best_params_)
    clf.fit(X, y)
    ...
    pd.DataFrame(res).to_csv('SVM_pred.csv', index=False, encoding="utf-8")
