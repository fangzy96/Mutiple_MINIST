...
def predict(model,vali_loader):
    ...
    # print(res[0].values)
    # print(predict)
    pd.DataFrame(res).to_csv('fnn_pred.csv',index=False,encoding="utf-8")
# print(model)
# save model
# torch.save(model.state_dict(),'cnn_test_model.pth')

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run the FNN:')
    ...
    # define the model
    model = FNN(num_feature=num_feature, classes=classes).to(device)
    ...
    # plt.axhline(y=np.mean(acc),color='r')
    plt.xlabel('Folds')
    plt.ylabel('Accuracy')
    plt.ylim((0.8, 1.0))
    plt.grid()
